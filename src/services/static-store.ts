/**
 * Store identity for a Saleor storefront.
 *
 * Saleor has no notion of the record this storefront calls a "store" — name, logo, favicon,
 * currency, menus, plugin toggles, theme variables. On the storefront's REST API that comes from
 * `/api/stores/public-details`; there is no such API here, so the storefront supplies it from its
 * own config and registers it once at boot.
 *
