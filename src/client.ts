...
/**
 * SaleorConnector — server-side entry point. kitcommerce-core creates one per
 * request with a custom fetch (carrying cookies); client-side it imports the
 * prebuilt singleton services instead (exported below). Credentials are injected
 * once via the static setCredentials (from the SvelteKit client + server hooks).
 */
export class SaleorConnector {
  ...
  /** Inject api url / channel / store / tokens (client + server hook init). */
  static setCredentials(creds: Partial<Credentials>): void {
    BaseService.setCredentials(creds)
  }
}

export function createClient(fetchFn?: typeof fetch): SaleorConnector {
  return new SaleorConnector(fetchFn)
}

// Prebuilt singletons for client-side imports.
export {
  addressService,
  authService,
  autocompleteService,
